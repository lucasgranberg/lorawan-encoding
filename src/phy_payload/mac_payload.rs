...
pub fn encode_uplink<'a>(
    ...
    let mut pos = 1;
    buf[pos..pos + size_of::<FHDR>()].copy_from_slice(f_hdr.as_bytes());
    pos += size_of::<FHDR>();
...
mod tests {
    ...
    fn encode_empty_uplink() {
        let mut buf = [0u8; 255];
        let uplink =
            encode_uplink(&mut buf, true, [0, 1, 2, 3], true, true, 4, 5, &[], &[]).unwrap();
